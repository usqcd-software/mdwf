...
char *op_a_name = "1";
char *op_b_name = "inverse(mlib:conj(B)) * mlib:conj(B)";
   ...
   const struct QX(Fermion) *s)
{
    ...
    qx(op_Bx)(r->even, &state->even, params, s->even, &flops);
    qx(op_Bx)(r->odd, &state->odd, params, s->odd, &flops);
	      
}

static void
B1x(struct QX(Fermion) *r,
    const struct Q(Parameters) *params,
    const struct QX(Fermion) *s)
{
    struct Q(State) *state = params->state;
    ...
    qx(op_B1x)(r->even, &state->even, params, s->even, &flops);
    qx(op_B1x)(r->odd, &state->odd, params, s->odd, &flops);
}
...
operator_b(void)
{
    ...
    Bx(fermion_x, params, fermion_a);
    B1x(fermion_y, params, fermion_x);
