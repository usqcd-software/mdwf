...
main(int argc, char *argv[])
{
    ...
    zprint("seeds: U %d a %d b %d", seed_u, seed_a, seed_b);
    ...
    if (QOP_MDWF_import_fermion(&fermion_a, state, read_fermion_a, NULL)) {
	zprint("Importing fermion A failed");
	goto no_fermion_a;
    }
    if (QOP_MDWF_import_fermion(&fermion_b, state, read_fermion_b, NULL)) {
	zprint("Importing fermion B failed");
	goto no_fermion_b;
    }
...
no_params:
    QOP_MDWF_free_fermion(&fermion_b);
no_fermion_b:
    QOP_MDWF_free_fermion(&fermion_a);
no_fermion_a:
    QOP_MDWF_fini(&state);
end:
    QMP_finalize_msg_passing();
