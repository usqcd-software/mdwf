...
char *op_a_name = "mlib:conj(B)";
char *op_b_name = "mlib:B";
  ...
  const struct QX(Fermion) *s)
{
    ...
    qx(op_B)(r->even, &state->even, params, s->even, &flops);
    qx(op_B)(r->odd, &state->odd, params, s->odd, &flops);
