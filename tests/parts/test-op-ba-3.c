...
static void
BA1(struct QX(Fermion) *r,
    const struct Q(Parameters) *params,
    const struct QX(Fermion) *s)
{
    struct Q(State) *state = r->state;
    long long flp = 0;

    qx(op_BA1)(r->even, &state->even, params, s->even, &flp);
    qx(op_BA1)(r->odd, &state->odd, params, s->odd, &flp);
}
...
operator_a(void)
{
    ...
    BA1(fermion_x, params, fermion_a);
