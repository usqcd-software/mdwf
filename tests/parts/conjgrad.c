...
run_it(struct QOP_MDWF_State *state, struct QOP_MDWF_Parameters *params)
{
    ...
    cg(R, &out_iter, &out_eps,
       ...
       QOP_MDWF_LOG_CG_RESIDUAL | QOP_MDWF_LOG_DIRAC_RESIDUAL);

    QOP_MDWF_free_fermion(&R);
    QOP_MDWF_free_fermion(&guess);
    QOP_MDWF_free_fermion(&rhs);
    QOP_MDWF_free_gauge(&U);
    zprint("%s: end max_iter=%d, esp=%g", name, max_iter, eps);
    return 0;
...
main(int argc, char *argv[])
{
	...
	zprint("Usage: conjgrad M m sB sC Nx Ny Nz Nt Lx Ly Lz Lt Ls sU sX sY");
    ...
    if (run_it(mdwf_state, mdwf_params)) {
	zprint("test failed");
	goto end;
    }
